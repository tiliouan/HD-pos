...
class PaymentDialog(QDialog):
    ...
    def process_payment(self):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                # Add receipt options to payment details
                self.payment_details['print_thermal'] = self.thermal_receipt_check.isChecked()
                self.payment_details['generate_pdf'] = self.pdf_invoice_check.isChecked()
                self.payment_details['auto_print'] = self.auto_print_check.isChecked()
                
                # Generate receipts based on options
                if self.payment_details['print_thermal'] or self.payment_details['generate_pdf']:
    ...
    def generate_receipts(self):
        """Generate receipts based on selected options."""
        try:
            # Generate thermal receipt if requested
            if self.payment_details['print_thermal']:
                receipt_text = self.receipt_generator.generate_thermal_receipt(self.sale, self.payment_details)
                
                # Save receipt text
                receipt_path = self.receipt_generator.save_receipt_text(receipt_text, str(self.sale.sale_id))
                self.payment_details['thermal_receipt_path'] = receipt_path
                
                # Auto-print if enabled
                if self.payment_details['auto_print']:
                    success = self.receipt_generator.print_thermal_receipt(receipt_text)
                    if success:
                        self.logger.info("Thermal receipt printed automatically")
                    else:
                        QMessageBox.warning(self, "Print Warning", 
                                          "Receipt saved but could not be printed automatically.")
            
            # Generate PDF invoice if requested
            if self.payment_details['generate_pdf']:
                pdf_path = self.receipt_generator.generate_pdf_invoice(self.sale, self.payment_details)
                self.payment_details['pdf_invoice_path'] = pdf_path
                
                QMessageBox.information(self, "PDF Generated", 
                                      f"Invoice saved as: {pdf_path}")
                
        except Exception as e:
            self.logger.error(f"Error generating receipts: {e}")
            QMessageBox.warning(self, "Receipt Error", 
                              f"Payment completed but receipt generation failed: {e}")
